...
:: Exclude these folder names (case-insensitive)
set EXCLUDE_DIRS=__pycache__ venv tests .idea .pytest_cache node_modules
...
:: Process files first - different handling for root vs subdirectories
for %%f in ("%current_dir%\*") do (
    if not "%%~nxf"=="%~nx0" (
        if "%current_dir%"=="." (
            echo %~dp0%%~nxf
        ) else (
            echo %~dp0!parent_path:~1!\%%~nxf
        )
    )
)
